...
- **시장가 매수는 현금이 닿는 데까지 삽니다.** 최우선 매도호가로 살 수 있는 수량을 넘겨 보내면 거부하지 않고
  그 수량까지만 삽니다(응답의 `qty` 가 줄어 있습니다). 한 단위도 못 살 때만 `INSUFFICIENT_CASH` 입니다.
  화면의 `maxBuyQty` 를 그대로 보내면 됩니다.
...
```jsonc
{
  ...
  "tradable": [{                 // 참여 종목 전체. 지금 낼 수 있는 최대 수량
    ...
    "maxBuyQty": 2210,           // 최우선 매도호가 기준(없으면 현재가), 수수료 포함해서 살 수 있는 최대 (lotSize 배수)
...
- **`tradable` 을 수량 입력의 상한으로 쓰세요.** 부하 테스트에서 거부된 주문의 **83%가 `INSUFFICIENT_CASH`** 였습니다. 화면이 직접 계산하면 수수료·호가단위 때문에 틀리기 쉽습니다. `maxBuyQty` 는 **최우선 매도호가 기준**입니다(없으면 현재가). 시장가로 그대로 보내면 됩니다. 지정가를 그보다 높게 넣는 화면이라면 그만큼 줄여야 합니다.
