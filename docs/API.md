...
| `cashInterestPerMin` | 0 | 0~0.1 | 보유 현금에 월급 주기로 붙는 이자(분당). 투입액이 아니라 수익으로 잡힌다. 기본 0 |
| `openPremium` | 0 | 0~2 | 시초가 프리미엄. 0 이 아니면 적정가·시초가가 기준가(공모가)보다 이만큼 높게 열려 공모 참여자만 그 차이를 얻는다. 시장조성자 경제 프리셋의 일부 |
| `marketMaker` | enabled **false** / band 0.005 / lagSec 180 / slice 0.10 | band 0.0005~0.5, lagSec 0~3600, slice 0.001~1 | 시장조성자. 켜면 실권주를 든 시스템 참가자가 기준가 ±band 에 양방향 호가를 댄다(순위·월급·`playerCount` 제외). 기준가는 적정가를 lagSec 시정수로 따라간다. slice 는 1회 호가량(발행량 대비) |
...
**설정 묶음(프리셋).** `GET /api/meta` 의 `presets` 에 `src/config.js` 의 `PRESETS` 가 그대로 옵니다. 지금은 `marketMaker` 하나 —
'아무거나 막 사기'가 하위권이 되는 시장조성자 경제(무상증자 0 · 시초가 프리미엄 30% · 시장조성자 켬 · 인플레 0.3% · 발행 300% ·
뉴스 45초/±25% · 잡음 0.02 · 호가 사다리 2단계). 방을 만들 때 `config` 에 `presets.marketMaker` 를 펼쳐 넣으면 됩니다:
`{ "name": "…", "config": { ...presets.marketMaker, "stockCodes": [...] } }`. 기본 경제와의 차이와 대가는 README '밸런스에 관한 기록'에 있습니다.
...
```jsonc
{
  ...
  "playerCount": 32,             // 봇 포함. 시장조성자(시스템 참가자)를 켠 방이어도 그건 빠진다
  "humanCount": 2,
  ...
  "config": { "lotSize": 10, "feeRate": 0.0015, "seedMoney": 1000000,
              ...
              "inflationPerMin": 0.005, "maxBonusRate": 0.05, "openPremium": 0,
              "durationMin": 15, "ipoSec": 45 },

  "stocks": [{
    "code": "SEC", "name": "삼성전자", "color": "#1b3f7a",
    "last": 1105,                // 최종 체결가 — 체결로만 움직인다
    "fair": 1396,                // 적정가 — 인플레이션과 개별 실적이 반영된 이론가(공개 정보)
    "open": 1110, "high": 1110, "low": 1100,   // 시초가. openPremium 을 켠 방에서는 공모가가 아니라 적정가에서 열린다
    "ipoPrice": 1110,            // 공모가(배정가). 공모가 끝나면 확정된다. 기본 경제에서는 open 과 같다
    "volume": 1310,
    "float": 4000,               // 발행 수량(공모 배정분 + 안 팔린 실권주)
    "issued": 7371,              // 실제 유통 주식 수(무상증자로 늘어난다. 시장조성자를 켠 방에서는 실권주 매각분만큼도)
...
- `depth.bids/asks` 는 **비어 있을 수 있습니다.** 특히 매도 호가는 전체 시간의 약 20% 동안 빕니다(사람 33명 방에서는 49%). 빈 상태를 정상으로 그려주세요. 시장조성자를 켠 방에서는 실권주가 남아 있는 한 거의 늘 채워집니다(실측 공백 0.2%).
...
서버는 두 조건 중 하나라도 맞는 방을 걷어냅니다. 기준은 조건마다 다르고, **1분마다** 확인합니다.

| | |
|---|---|
| 마감된 방 | 마감된 시점부터 **30분** (`ROOM_ENDED_MS`) |
| 버려진 방 | 마지막 **요청**부터 **4시간** (`ROOM_IDLE_MS`) — 시작 안 한 로비도 포함 |

| 환경변수 | 기본값 | |
|---|---|---|
| `ROOM_ENDED_MS` | `1800000` (30분) | 마감된 방을 보존하는 시간. 결과·CSV 를 받아갈 수 있는 시한입니다 |
| `ROOM_IDLE_MS` | `14400000` (4시간) | 활동 없는 방의 기준 시간. 훑는 주기는 두 기준 중 짧은 쪽의 절반(최대 1분)입니다. 0·음수·숫자가 아닌 값은 무시하고 기본값을 씁니다 |
...
게임 틱은 이와 별개로 **마감되는 즉시** 멈춥니다. 걷히기까지 30분 동안 도는 게 아닙니다.
...
같은 이유로 '아무거나 계속 사서 들고 있기'도 보상받습니다 — 이 경제에서는 매분 월급을 아무 종목에나 넣는 전략이
33명 중 4등 안팎이고 값 조정으로는 안 내려갑니다(README '밸런스에 관한 기록').

**시장조성자 경제(선택, `presets.marketMaker`).** 그걸 바꾸고 싶은 방을 위한 설정 묶음입니다. 무상증자를 끄고(`bonus` 알림이
안 옵니다), 시초가를 적정가 = 기준가 + 30%(`openPremium`)에서 열어 공모 참여자만 그 차이를 얻게 하고, 공모에서 안 팔린 실권주를 든
**시장조성자**(시스템 참가자 — 순위·월급·`playerCount` 제외, 호가만 걸고 남의 주문은 안 먹음)가 적정가 기준가 ±0.5% 에 양방향 호가를 댑니다.
기준가는 적정가를 180초 시정수로 천천히 따라가므로 뉴스로 적정가가 뛰어도 호가는 옛 값 근처에서 시작해 몇 분에 걸쳐 올라갑니다.
화면이 알아야 할 것: 개장 직후 보유 종목의 등락률(시초가 대비)은 0% 인데 평가손익(공모가 대비)은 +30% 로 보이는 게 정상이고,
공모 단계에는 전 종목 괴리율이 −23% 로 뜨며(현재가 = 기준가, 적정가 = 기준가 +30%), `snapshot.config.openPremium` 으로 켜졌는지 알 수 있습니다.
기본 화면(`public/index.html`)은 이 값을 보고 안내 문구를 바꿉니다.
...
**돌발뉴스.** 평균 70초마다 종목별 호재/악재가 터집니다(15분에 약 12건, 호재:악재 ≈ 5:5. 시장조성자 프리셋은 45초·적정가 충격 ±25%·효력 180초).
